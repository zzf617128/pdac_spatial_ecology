...
dataset_dir <- file.path(root, "datasets")
dir.create(table_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(doc_dir, recursive = TRUE, showWarnings = FALSE)

slides <- data.table(
  slide = c("slide1", "slide2", "slide3", "slide4", "slide5", "slide5b2", "slide5b3", "slide6"),
  gsm = c("GSM9294399", "GSM9294400", "GSM9294401", "GSM9294402", "GSM9294404", "GSM9294405", "GSM9294406", "GSM9294403")
)

slides[, recovered_patient_id := "not recovered"]
slides[, recovered_specimen_id := "not recovered"]
slides[, recovered_sample_id := paste(gsm, slide, sep = " / ")]
slides[, treatment_group := "treatment naive"]
slides[, tissue_source := "PDAC tumor resection; FFPE 5 um section"]
slides[, tissue_block := "not recovered"]
slides[, evidence_source := paste(
  "GSE310352 series matrix / family SOFT / MINiML;",
  "GSM supplementary file names;",
  "processed cell_metadata.csv.gz headers;",
  "GSE310353/GSE282302/GSE310388 family SOFT context",
  sep = " "
)]
slides[, evidence_details := paste(
  "GEO records provide slide title, GSM accession, tissue = PDAC tumor resection, and treatment naive;",
  "processed metadata fields are fov/cell_ID/area/coordinates/IF intensities only;",
  "no patient, specimen, block, donor, case, or anonymized patient key was found."
)]
slides[, confidence := "not recovered"]
slides[, current_cosmx_support_patient_specimen_level := FALSE]
slides[, recommended_interpretation_unit := "slide/FOV level only"]
slides[, notes := "Do not interpret current GSE310352 CosMx support as patient- or specimen-level replicated unless an external sample key is obtained from authors or publisher supplement."]

slides[slide %in% c("slide5", "slide5b2", "slide5b3"),
  notes := paste(
    notes,
    "The slide5/slide5b2/slide5b3 names suggest a possible slide-family relationship, but this is only a filename clue and is insufficient for patient/specimen mapping."
  )
]

out_csv <- file.path(table_dir, "gse310352_slide_patient_mapping_attempt.csv")
fwrite(slides, out_csv)

soft_files <- file.path(dataset_dir, c(
  "GSE310352_series_matrix.txt.gz",
  "GSE310352_family.soft.gz",
  "GSE310352_family.xml.tgz",
  "GSE310353_family.soft.gz",
  "GSE282302_family.soft.gz",
  "GSE310388_family.soft.gz"
))

processed_files <- list.files(
  file.path(dataset_dir, "gse310352_processed"),
  pattern = "cell_metadata\\.csv\\.gz$|cell_by_gene_counts\\.csv\\.gz$",
  full.names = FALSE
)
...
report <- c(
  ...
  "",
  "## Sources Checked",
  paste0("- Local/official GEO metadata files: ", paste(basename(soft_files[file.exists(soft_files)]), collapse = ", "), "."),
  ...
  paste0("- Processed local GSE310352 files inspected: ", paste(processed_files, collapse = "; "), "."),
  ...
  "",
  "## Output",
  paste0("- Mapping attempt table: `", out_csv, "`.")
)

writeLines(report, file.path(doc_dir, "gse310352_patient_mapping_recovery_report.md"), useBytes = TRUE)

cat("Wrote:\n", out_csv, "\n", file.path(doc_dir, "gse310352_patient_mapping_recovery_report.md"), "\n", sep = "")
