...
DEFAULT_FILE = ROOT / "results" / "submission_package_v3_2026_07_01_submission_ready_only.zip"
...
def upload_file(bucket_url: str, token: str, path: Path) -> dict:
    ...
    try:
        ...
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"Zenodo file upload error {exc.code} for {path}: {detail}") from exc


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create a Zenodo draft deposition and upload the ED10 v3 submission-ready package."
    )
    ...
    print(f"Created Zenodo draft deposition: {dep_id}")
