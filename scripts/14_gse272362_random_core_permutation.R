...
if (is.na(n_perm) || n_perm < 100L) {
  stop("n_perm must be at least 100.")
}
...
write_status <- function(status, payload) {
  path <- file.path(project_root, "results", "logs", "stage_14_gse272362_random_core_permutation_status.json")
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
  payload <- c(list(stage = "14_gse272362_random_core_permutation", status = status, timestamp_utc = now_iso()), payload)
  to_json <- function(x) {
...
normalize_specimen_type <- function(x) {
  ...
  out[grepl("primary|tumou?r|pdac", y)] <- "primary_tumor"
...
median_neighbor_distance <- function(coords) {
  ...
  if (nrow(coords) > 1500) {
    sample_idx <- sample(sample_idx, 1500)
  }
  sampled <- coords[sample_idx, , drop = FALSE]
  FNN::get.knnx(data = sampled, query = sampled, k = 2)$nn.dist[, 2] |>
    median(na.rm = TRUE)
}
...
analyze_one_sample <- function(dt, n_perm) {
  ...
  sample_rows <- rbindlist(lapply(names(target_scores), function(target) {
    ...
    empirical_p_more_negative <- (1 + sum(null_values <= observed, na.rm = TRUE)) / (1 + sum(is.finite(null_values)))
    data.table(
      ...
      empirical_p_more_negative = empirical_p_more_negative
    )
  }))
...
summarize_results <- function(sample_results) {
  ...
  sample_results[
    ...
    by = .(dataset_id, specimen_type, target)
  ][order(specimen_type, target)]
}
...
plot_results <- function(sample_results, summary_results) {
  ...
  p1 <- ggplot(sample_results, aes(x = specimen_type, y = delta_vs_null_median, fill = specimen_type)) +
    geom_hline(yintercept = 0, linewidth = 0.35, color = "grey30") +
    geom_boxplot(outlier.shape = NA, alpha = 0.8) +
    geom_jitter(width = 0.12, size = 1.6, alpha = 0.75) +
    facet_wrap(~target, scales = "free_y") +
    labs(
      title = "GSE272362 random-core control",
      subtitle = "Negative delta means true CAF-myeloid core has stronger co-localization than random same-size cores",
      x = NULL,
  ...
  ggsave(file.path(out_dir, "gse272362_random_core_permutation_delta.png"), p1, width = 11, height = 6, dpi = 180)
  ggsave(file.path(out_dir, "gse272362_random_core_permutation_delta.pdf"), p1, width = 11, height = 6)

  p2 <- ggplot(summary_results, aes(x = specimen_type, y = n_observed_more_negative_than_null, fill = target)) +
    ...
    labs(
      title = "Samples where true CAF-myeloid core beats random cores",
      x = NULL,
      y = "Number of samples"
    ) +
    theme_bw(base_size = 11) +
    theme(axis.text.x = element_text(angle = 25, hjust = 1), plot.title = element_text(face = "bold"))
  ggsave(file.path(out_dir, "gse272362_random_core_permutation_counts.png"), p2, width = 10, height = 5.5, dpi = 180)
  ggsave(file.path(out_dir, "gse272362_random_core_permutation_counts.pdf"), p2, width = 10, height = 5.5)
}

main <- function() {
  set.seed(20260624)
  spot_path <- file.path(project_root, "results", "tables", "gse272362_rds_spot_level_scores.csv")
  if (!file.exists(spot_path)) stop("Missing spot-level score table: ", spot_path)
  message("Reading ", spot_path)
  spot <- fread(spot_path)
  spot[, specimen_type := normalize_specimen_type(specimen_type)]
  required <- c("sample_id", "specimen_type", "x_pixel", "y_pixel", "score_caf_myeloid_barrier", target_scores)
  missing <- setdiff(required, names(spot))
  if (length(missing) > 0) stop("Missing required columns: ", paste(missing, collapse = ", "))

  sample_keys <- unique(spot[, .(sample_id, specimen_type)])
  ...
  for (i in seq_len(nrow(sample_keys))) {
    ...
    message(sprintf("[%d/%d] %s", i, nrow(sample_keys), sid))
    results[[i]] <- analyze_one_sample(spot[sample_id == sid], n_perm = n_perm)
  }
  ...
  fwrite(sample_results, file.path(out_dir, "gse272362_rds_random_core_permutation_sample_stats.csv"))
  fwrite(summary_results, file.path(out_dir, "gse272362_rds_random_core_permutation_summary.csv"))
  fwrite(null_results, file.path(out_dir, "gse272362_rds_random_core_permutation_null_rhos.csv"))
  plot_results(sample_results, summary_results)

  write_status("success", list(
    n_samples = length(unique(sample_results$sample_id)),
    n_perm = n_perm,
    output_sample_stats = file.path(out_dir, "gse272362_rds_random_core_permutation_sample_stats.csv"),
    output_summary = file.path(out_dir, "gse272362_rds_random_core_permutation_summary.csv")
  ))
  message("Done.")
}
